...
class ObjectTracker:
    ...
    Why this helps
    ──────────────
    The depth camera always returns the *visible surface* point, which shifts
    when the robot views the object from a different angle. By maintaining a
    per-instance filtered position across frames the tracker progressively
    converges toward a stable world-frame estimate, dampening the per-frame
    surface-point jitter.

    Parameters
    ──────────
    max_distance : float
        Maximum distance (metres) between a new detection and an existing
        track before the detection is treated as a *new* object instance.
    max_age : float
        Seconds of silence before a track is discarded.
    alpha : float  (0 < alpha ≤ 1)
        EMA weight given to the newest observation.
        Low  (e.g. 0.1) → slow, very smooth  – good for slow-moving objects.
        High (e.g. 0.5) → fast, less smooth  – follows position changes quickly.
    """
    ...
    def update(self, class_name: str, raw_position: np.ndarray) -> np.ndarray:
        ...
        Returns the *filtered* position that should be published.
        """
        self._prune_stale()
        ...
        # First time we see this instance → register a new track
...
def estimate_surface_depth(
    ...
    """
    Return a robust depth estimate for a detection bounding box.

    Strategy
    ────────
    1.  Crop the *full* bounding-box ROI from the depth image (previously
        only a tiny centre window was used, which is sensitive to the exact
        pixel chosen as the projection point).
    2.  Keep only valid pixels (> 0 and ≤ max_depth).
    3.  Return the value at *depth_percentile* of the valid distribution.

    Using a low percentile (e.g. 20) favours the near surface of the object
    while ignoring background pixels that sneak into the bbox. This gives a
    more consistent reference depth regardless of viewing angle than the
    raw centre-pixel median.

    Returns None if no valid depth pixels exist in the ROI.
    """
    h, w = depth_img.shape
    u1 = max(0, int(x1))
    v1 = max(0, int(y1))
...
class YoloDepthDetectorNode(Node):
    def __init__(self):
        ...
        # Camera intrinsics (override CameraInfo topic when all > 0)
        ...
        # Depth sampling parameter
        # Lower  → closer to nearest visible surface (more stable w.r.t. angle).
        # Higher → deeper into the object volume (better centre estimate if bbox
        #          contains both front and back surfaces, e.g. thin objects).
    ...
    def sync_callback(self, rgb_msg: Image, depth_msg: Image) -> None:
        ...
        for box in results.boxes:
            ...
            # ── Depth estimation (full bbox, percentile-based) ────────────────
            # Using the entire bounding-box ROI and a low percentile gives a
            # more angle-stable depth estimate than the small centre window:
            #   • It is not sensitive to which exact pixel happens to be at the
            #     projected centre of the detection.
            #   • A low percentile favours the near-surface of the object while
            #     rejecting background pixels that leak into the bbox.
            ...
            # Bounding-box centre pixel for back-projection
            ...
            # ── Spatial tracker: EMA filtering ───────────────────────────────
            # Each unique object instance gets its own track.  The EMA smooths
            # the per-frame surface-point jitter caused by viewing angle changes.
            # Detections more than `tracker_max_distance` metres away from any
            # existing track of the same class start a new track (multi-instance
            # support, e.g. two chairs in the scene).
            ...
            world_msg.header.frame_id    = cls_name          # object identity
    ...
    def _parse_depth(self, depth_msg: Image) -> Optional[np.ndarray]:
        """Convert depth image to float32 metres (supports 16UC1 and 32FC1)."""
        try:
