...
class CommanderNode(Node):
    def __init__(self):
        ...
        #act clients
        self.navigate_client = ActionClient(self, NavigateToObject, '/navigate_to_object')
        ...
        self.navigate_client.wait_for_server(timeout_sec=10.0)
        self.pick_client.wait_for_server(timeout_sec=10.0)
        self.place_client.wait_for_server(timeout_sec=10.0)
...
""")
    ...
    def _do_navigate(self, params):
        ...
        if position:
            goal.target_position = [float(position[0]), float(position[1]), float(position[2])]
        ...
        while not future.done() and time.time() - start < 5.0:
            time.sleep(0.01)
        if not future.done():
            return {"success": False, "message": "Navigate timeout"}
        
        goal_handle = future.result()
        if not goal_handle.accepted:
            return {"success": False, "message": "Navigate rejected"}
        ...
        if not result_future.done():
            return {"success": False, "message": "Navigate result timeout"}
